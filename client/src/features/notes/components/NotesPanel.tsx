...
const stripHtmlTags = (htmlContent: string): string => {
  ...
  if (typeof window === "undefined" || !window.DOMParser) {
    return (
      htmlContent.replace(/<\/?[^>]+(>|$)/g, "").trim() || "No additional text"
    );
  }
  try {
              ...
              <div className="relative flex h-3.5 w-3.5 items-center justify-center">
                <AnimatePresence mode="wait" initial={false}>
                  {isLoading ? (
                    ...
                    >
                      <Loader2 className="h-3.5 w-3.5 animate-spin stroke-[2.2]" />
                    </motion.div>
                  ) : (
                    <motion.div
                      key="icon"
            ...
            {isLoading && notes.length === 0 && (
              ...
              >
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-transparent p-4 bg-white/60 space-y-2 animate-pulse"
                  >
                ...
                {filteredNotes.map((note) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <motion.div
                        layout="position"
                        className="flex items-center justify-between gap-3 relative z-10 w-full"
                      >
                        <h3
                      ...
                      <motion.p
                        layout="position"
                        className="text-[11px] leading-relaxed text-slate-400 line-clamp-2 pointer-events-none relative z-10 font-normal w-full"
                      >
                        {note.cleanSnippet}
                      </motion.p>
                    </motion.div>
