...
export const NotebookDashboard: React.FC = () => {
  ...
  // State switcher to swap views inside the center pane
  const [currentView, setCurrentView] = useState<"notebooks" | "tasks">("notebooks");

  return (
    <div className="flex w-full h-screen bg-[#F8FAFC] text-slate-800 antialiased font-sans overflow-hidden select-none">
      
      {/* 1. LEFT SIDEBAR COMPONENT (Static) */}
      <aside className="w-64 border-r border-slate-200/60 bg-white flex flex-col justify-between p-6 shrink-0 h-full z-20">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full">
        ...
        {/* Top Header Controls (Static) */}
        <header className="h-16 border-b border-slate-200/60 bg-white flex items-center justify-between px-8 shrink-0">
          ...
          <div className="flex items-center gap-4 w-full max-w-md mx-4">
            <div className="relative w-full">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200/80 rounded-xl pl-9 pr-4 py-1.5 text-sm focus:outline-none focus:border-slate-300 transition-colors placeholder:text-slate-400"
              />
            </div>
          </div>

          <div className="flex items-center gap-2">
            <button className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200/80 rounded-xl text-xs font-medium text-slate-500 bg-slate-50 hover:bg-slate-100 transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-8 max-w-5xl w-full mx-auto h-full [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {currentView === "notebooks" && (
            <div className="animate-[fadeIn_0.3s_ease-out]">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Notebook Spaces</h1>
                  <p className="text-xs text-slate-400 mt-0.5">Select a primary study node container.</p>
                </div>
                <button className="flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-medium hover:bg-slate-800 transition-all shadow-md shadow-slate-900/10">
                ...
                {notebooks.map((notebook) => {
                  ...
                  return (
                    <div
                      ...
                      className="group relative rounded-[2rem] p-6 border border-white/40 shadow-[inset_0_2px_4px_rgba(255,255,255,0.6),_0_4px_20px_rgba(0,0,0,0.01)] hover:shadow-[inset_0_2px_4px_rgba(255,255,255,0.6),_0_12px_30px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all duration-300 flex flex-col justify-between min-h-[200px] cursor-pointer"
                    >
                      <div>
                        <div className="flex justify-between items-center mb-6">
                          <div className="p-2.5 rounded-xl bg-white shadow-[0_4px_10px_rgba(0,0,0,0.01)] border border-slate-100" style={{ color: theme.accent }}>
                        ...
                        {noteCount > 0 && (
                          <ul className="space-y-1.5 mb-4 opacity-75">
                            {notebook.notes.slice(0, 5).map((note) => (
                      ...
                      <div className="mt-auto flex items-center justify-between pt-3 border-t border-slate-200/30 text-[11px] text-slate-400">
